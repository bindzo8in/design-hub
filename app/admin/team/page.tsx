...
import { getTeamMemberColumns } from "@/features/team/components/team-columns";
import { useTeamMembersQuery, useDeleteTeamMemberMutation } from "@/features/team/hooks/use-team-query";
...
export default function TeamMembersAdminPage() {
  const { data: teamMembers = [], isLoading, error } = useTeamMembersQuery();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-[#101735]/40 border border-[#26336F]/20 p-6 rounded-3xl backdrop-blur-xl">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Manage agency designers, developers, product managers, and internal staff records.
          </p>
        </div>
        <Button
          asChild
        ...
        <AlertDialogContent className="bg-[#101735] border border-[#26336F]/30 text-white rounded-2xl max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-400 text-sm">
              This action cannot be undone. Removing a team member will permanently delete their internal record and profile references.
            </AlertDialogDescription>
          </AlertDialogHeader>
